...
- **Owner & Manager Dual Precache Circuit Breaker (SW v15 - Resilient Zero-Latency Engine)**: Service worker runs an origin-isolated network timeout race for page navigations and RSC flight streams, prioritizing live online responses with a generous 15-second safety ceiling (preventing premature 2.5s aborts during production serverless cold starts). Pre-caches static shell assets (`/`, manifest, icons, logo) on install into `optical-manager-cache-v15` without heavy SSR routes, eliminating concurrent server compilation storms on startup while caching visited pages dynamically at runtime. Uncached dynamic RSC flight requests (`headers.get("RSC") === "1"` or `_rsc`) return clean `503` status responses instead of `307` Location redirects, completely preventing raw RSC flight JSON payloads from displaying on blank screens. The offline HTML fallback strictly includes `charset=utf-8` to ensure correct rendering of icons and symbols (`⚡`).
- **Resilient Server Component Timeouts & Concurrent Queries**: Database queries across all Shop and Owner server routes execute concurrently with generous 8,000ms timeout guards and try/catch fallback datasets, ensuring server rendering reliably connects to serverless Neon Postgres databases even during cold connection wake-ups without throwing uncaught exceptions.
